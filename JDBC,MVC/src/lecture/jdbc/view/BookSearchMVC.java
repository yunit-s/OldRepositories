...
//	private static BasicDataSource basicDS; // 이거는 커넥션 풀 만드는 거야. DAO 로 가야해. view에는 필요 없어.
...
//	}
	...
	public void start(Stage primaryStage) throws Exception {
		...
		textField.setOnAction(e -> {
			// 이벤트를 처리하는 부분. 그런데 view에서는 이벤트를 처리하는 로직을 직접 구현하면 안 돼.
			// Controller 를 이용해서 Service에 로직 실행 요청을 해야 한다.
			// Controller 객체 생성
			BookSearchByKeywordController controller = new BookSearchByKeywordController();
			
			// 컨트롤러에게 기본 정보를 전달해주고, 그 결과값으로 TableView에 넣을 list를 리턴받음
