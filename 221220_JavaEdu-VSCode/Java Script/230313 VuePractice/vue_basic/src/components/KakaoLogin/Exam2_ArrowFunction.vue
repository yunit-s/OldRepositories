<template>
  <div class="exam2_arrowfunction">
    <h3>Exam2_ArrowFunction.vue 출력</h3>
    <div>참고 사이트</div>
    ...
    <div>loginEmail : {{ this.loginEmail }}</div>
    <div>loginNickname : {{ this.loginNickname }}</div>
  </div>
</template>

<script>
export default {
  name: "Exam2_ArrowFunction",
  data() {
    return {
      loginEmail: "",
      loginNickname: ""
  ...
  mounted() {
    ...
    this.loginEmail = "로그인 전 이메일";
    this.loginNickname = "로그인 전 닉네임";
        ...
        success: (response) => {
          ...
          window.Kakao.API.request({
            ...
            success: (response) => {
              ...
              this.loginEmail = response.kakao_account.email;
              this.loginNickname = response.kakao_account.profile.nickname;
            },
            fail: (error) => {
              console.log(error);
            },
